...
# Definición estricta de columnas para las hojas de cálculo
PATIENT_COLUMNS = ["id", "name", "phone", "lesion", "review_date", "revisions", "is_deleted"]
EXERCISE_COLUMNS = ["id", "name", "patientName", "videoUrl", "category", "defaultNote", "is_deleted"]
PLAN_COLUMNS = ["id", "patientId", "title", "exerciseIds", "exerciseInstructions", "generalInstructionIds", "pin", "startDate", "isActive", "is_deleted"]
PROGRAM_COLUMNS = ["id", "patientId", "title", "frequency", "duration", "generalInstructionIds", "generalNote", "pin", "startDate", "daysData", "is_deleted"]
INSTRUCTION_COLUMNS = ["id", "text", "is_deleted"]
...
<style>
    ...
    div[class*="st-key-default_note_"] textarea {
        border: 2px solid #13765d !important; border-radius: 18px 18px 18px 5px !important; background: #fffef7 !important;
        padding: 14px 16px !important; box-shadow: 0 4px 12px rgba(16, 61, 51, 0.10) !important; min-height: 110px !important; position: relative !important;
    }
    div[class*="st-key-default_note_"] textarea:focus { border-color: #103d33 !important; box-shadow: 0 0 0 2px rgba(19, 118, 93, 0.12), 0 5px 14px rgba(16, 61, 51, 0.12) !important; }
    div[class*="st-key-default_note_"] { position: relative !important; }
...
if 'logged_pin' not in st.session_state: st.session_state.logged_pin = None
if 'editing_sesion_id' not in st.session_state: st.session_state.editing_sesion_id = None
if 'editing_program_id' not in st.session_state: st.session_state.editing_program_id = None
if 'failed_login_attempts' not in st.session_state: st.session_state.failed_login_attempts = 0
if 'login_locked_until' not in st.session_state: st.session_state.login_locked_until = None
if 'confirm_delete_patient_id' not in st.session_state: st.session_state.confirm_delete_patient_id = None
if 'confirm_delete_session_id' not in st.session_state: st.session_state.confirm_delete_session_id = None
if 'confirm_delete_program_id' not in st.session_state: st.session_state.confirm_delete_program_id = None
...
def parse_bool(val):
    return str(val).strip().lower() in ("true", "1", "si", "sí", "t")
...
def ordinal_revision(n):
    return f"{ORDINALES_ES[n-1]} revisión" if 1 <= n <= len(ORDINALES_ES) else f"Revisión {n}"

def tiene_video_valido(url):
    return str(url).strip().lower().startswith("http")

# =============================================================
# MOTOR DE ACTUALIZACIONES ATÓMICAS Y AUTO-MIGRACIÓN
# =============================================================
def _col_letter(n):
    letters = ""
    while n > 0:
...
def get_ws_headers(worksheet_name, default_columns_tuple):
    """Auto-migra la hoja si le faltan columnas nuevas (como is_deleted)"""
    try:
        ws = get_ws(worksheet_name)
        headers = ws.row_values(1)
        if not headers:
            headers = list(default_columns_tuple)
            ws.append_row(headers, value_input_option="USER_ENTERED")
            return headers
        
        missing = [c for c in default_columns_tuple if c not in headers]
        if missing:
            headers.extend(missing)
            rango = f"A1:{_col_letter(len(headers))}1"
            ws.update(rango, [headers])
        return headers
    except Exception:
...
# =============================================================
# FORMATO Y UPSERT ESPECÍFICO POR ENTIDAD (CRUD Atómico)
# =============================================================
def format_patient(p):
    return {
        "id": p["id"], "name": p["name"], "phone": p.get("phone", ""),
        "lesion": p.get("lesion", ""), "review_date": p.get("review_date", ""),
        "revisions": json.dumps(p.get("revisions", [])),
        "is_deleted": format_bool(p.get("is_deleted", False))
    }

def upsert_patient(p):
    if st.session_state.gsheets_read_error: return False
    ok = atomic_upsert_row_by_dict("pacientes", "id", p["id"], format_patient(p), PATIENT_COLUMNS)
    if ok: st.cache_data.clear()
    return ok

def format_plan(pl):
    return {
        "id": pl["id"], "patientId": pl["patientId"], "title": pl["title"],
        "exerciseIds": json.dumps(pl["exerciseIds"]), 
        "exerciseInstructions": json.dumps(pl["exerciseInstructions"]),
        "generalInstructionIds": json.dumps(pl.get("generalInstructionIds", [])),
        "pin": pl["pin"], "startDate": pl.get("startDate", ""),
        "isActive": format_bool(pl.get("isActive", True)),
        "is_deleted": format_bool(pl.get("is_deleted", False))
    }

def upsert_plan(pl):
    if st.session_state.gsheets_read_error: return False
    ok = atomic_upsert_row_by_dict("sesiones", "id", pl["id"], format_plan(pl), PLAN_COLUMNS)
    if ok: st.cache_data.clear()
    return ok

def format_program_af(pr):
    return {
        "id": pr["id"], "patientId": pr["patientId"], "title": pr["title"],
        "frequency": pr["frequency"], "duration": pr["duration"],
        "generalInstructionIds": json.dumps(pr.get("generalInstructionIds", [])),
        "generalNote": pr["generalNote"], "pin": pr["pin"],
        "startDate": pr.get("startDate", ""), "daysData": json.dumps(pr["daysData"]),
        "is_deleted": format_bool(pr.get("is_deleted", False))
    }

def upsert_program_af(pr):
    if st.session_state.gsheets_read_error: return False
    ok = atomic_upsert_row_by_dict("programas_af", "id", pr["id"], format_program_af(pr), PROGRAM_COLUMNS)
    if ok: st.cache_data.clear()
    return ok

def format_exercise(e):
    return {
        "id": e["id"], "name": e["name"], "patientName": e.get("patientName", ""),
        "videoUrl": e["videoUrl"], "category": e["category"], "defaultNote": e.get("defaultNote", ""),
        "is_deleted": format_bool(e.get("is_deleted", False))
    }

def upsert_exercise(e):
    if st.session_state.gsheets_read_error: return False
    ok = atomic_upsert_row_by_dict("ejercicios", "id", e["id"], format_exercise(e), EXERCISE_COLUMNS)
    if ok: st.cache_data.clear()
    return ok

def save_exercises_bulk(exercises_list):
    """Actualización en masa para ejercicios (Botón 'Guardar Todos los Cambios')"""
    if st.session_state.gsheets_read_error: return
    formatted = [format_exercise(e) for e in exercises_list]
    conn.update(spreadsheet=SHEET_URL, worksheet="ejercicios", data=pd.DataFrame(formatted, columns=EXERCISE_COLUMNS))
    st.cache_data.clear()

def format_instruction(i):
    return {"id": i["id"], "text": i["text"], "is_deleted": format_bool(i.get("is_deleted", False))}

def upsert_general_instruction(i):
    if st.session_state.gsheets_read_error: return False
    ok = atomic_upsert_row_by_dict("indicaciones_generales", "id", i["id"], format_instruction(i), INSTRUCTION_COLUMNS)
    if ok: st.cache_data.clear()
    return ok

def save_general_instructions_bulk(inst_list):
    if st.session_state.gsheets_read_error: return
    formatted = [format_instruction(i) for i in inst_list]
    conn.update(spreadsheet=SHEET_URL, worksheet="indicaciones_generales", data=pd.DataFrame(formatted, columns=INSTRUCTION_COLUMNS))
    st.cache_data.clear()
...
def get_admin_access_code():
    try: return normalize_access_code(st.secrets["FISIO_ADMIN_PIN"])
    except Exception: return ""

def get_existing_access_codes(active_plans, active_programs):
    used_codes = set()
    for item in active_plans + active_programs:
        code = normalize_access_code(item.get("pin", ""))
        if code: used_codes.add(code)
    return used_codes

def generate_unique_access_code(active_plans, active_programs):
    used_codes = get_existing_access_codes(active_plans, active_programs)
    for _ in range(100):
        code = "".join(secrets.choice(ACCESS_CODE_ALPHABET) for _ in range(ACCESS_CODE_LENGTH))
        if code not in used_codes: return code
    ...
    try:
        ...
        for _, r in df.iterrows():
            ...
            if isinstance(revisions_raw, str) and revisions_raw.strip().startswith("["):
                try: revisions = json.loads(revisions_raw)
                except Exception: revisions = []

            if not revisions:
                old_ana, old_ins = clean_str(r.get("anamnesis", "")), clean_str(r.get("inspeccion", ""))
                old_mov, old_fue = clean_str(r.get("movilidad", "")), clean_str(r.get("fuerza", ""))
                if old_ana or old_ins or old_mov or old_fue:
                    revisions = [{"date": "", "anamnesis": old_ana, "inspeccion": old_ins, "movilidad": old_mov, "fuerza": old_fue}]
            ...
            records.append({
                ...
                "revisions": revisions,
                "is_deleted": parse_bool(r.get("is_deleted", False))
            })
        return records
    except Exception:
...
def get_general_instructions():
    try:
        ...
        for _, r in df.iterrows():
            ...
            if text:
                records.append({"id": clean_str(r.get("id", "")), "text": text, "is_deleted": parse_bool(r.get("is_deleted", False))})
        return records
    except Exception:
        st.session_state.gsheets_read_error = True
        return []
...
def get_plans():
    try:
        ...
        for _, r in df.iterrows():
            ...
            try: ex_ids = [clean_str(x) for x in json.loads(ex_ids_raw)] if isinstance(ex_ids_raw, str) and ex_ids_raw.startswith("[") else []
            except: ex_ids = []

            inst_raw = r.get("exerciseInstructions", "{}")
            try: cleaned_insts = {clean_str(k): v for k, v in json.loads(inst_raw).items()} if isinstance(inst_raw, str) and inst_raw.startswith("{") else {}
            except: cleaned_insts = {}

            plans.append({
                "id": clean_str(r.get("id", "")), "patientId": clean_str(r.get("patientId", "")), 
                "title": clean_str(r.get("title", "")), "exerciseIds": ex_ids, 
                ...
                "pin": clean_str(r.get("pin", "")), "startDate": clean_str(r.get("startDate", "")),
                "isActive": parse_bool(r.get("isActive", True)),
                "is_deleted": parse_bool(r.get("is_deleted", False))
            })
        return plans
    except Exception:
...
def get_programs_af():
    try:
        ...
        for _, r in df.iterrows():
            ...
            try: days_data = json.loads(days_raw) if isinstance(days_raw, str) and days_raw.startswith("[") else []
            except: days_data = []

            programs.append({
                "id": clean_str(r.get("id", "")), "patientId": clean_str(r.get("patientId", "")),
                "title": clean_str(r.get("title", "")), "frequency": clean_str(r.get("frequency", "")),
                "duration": clean_str(r.get("duration", "")),
                "generalInstructionIds": json.loads(r.get("generalInstructionIds", "[]")) if isinstance(r.get("generalInstructionIds", "[]"), str) and r.get("generalInstructionIds", "[]").startswith("[") else [],
                "generalNote": clean_str(r.get("generalNote", "")), "pin": clean_str(r.get("pin", "")),
                "startDate": clean_str(r.get("startDate", "")), "daysData": days_data,
                "is_deleted": parse_bool(r.get("is_deleted", False))
            })
        return programs
    except Exception:
...
def get_checkins():
    try:
        ...
        for _, r in df.iterrows():
            records.append({
                "id": clean_str(r.get("id", "")), "planId": clean_str(r.get("planId", "")), 
                "date": clean_str(r.get("date", "")), "eva": clean_str(r.get("eva", "")), 
                "borg": clean_str(r.get("borg", "")), "comment": clean_str(r.get("comment", "")),
    ...
    try:
        ...
        for _, r in df.iterrows():
            if clean_str(r.get("planId", "")) == str(plan_id):
                ...
                try: checks = json.loads(raw_checks) if raw_checks.strip().startswith("[") else []
...
def guardar_progreso_sesion(plan_id, opened_at, checked_exercises):
    ...
    atomic_upsert_row_by_dict("progreso_sesion", "planId", str(plan_id), row_dict, PROGRESO_COLUMNS)
...
for _exercise in exercises: _exercise.setdefault("defaultNote", "")
general_instructions = get_general_instructions()
plans = get_plans()
programs_af = get_programs_af()
checkins = get_checkins()

# FILTRADO DE ENTIDADES ACTIVAS (Soft Delete)
active_patients = [p for p in patients if not p.get("is_deleted")]
active_exercises = [e for e in exercises if not e.get("is_deleted")]
active_plans = [pl for pl in plans if not pl.get("is_deleted")]
active_programs_af = [pr for pr in programs_af if not pr.get("is_deleted")]
active_instructions = [i for i in general_instructions if not i.get("is_deleted")]
...
def calcular_semaforo_paciente(paciente, todos_los_planes, todos_los_checkins):
    ...
    def parse_date(c):
        try: return datetime.datetime.strptime(c["date"], "%Y-%m-%d %H:%M")
    ...
    if len(checkins_pac) < 3:
        if planes_paciente: return resultado("AMARILLO", [f"Necesita al menos 3 reportes para calcular su adherencia (lleva {len(checkins_pac)})"])
    ...
    motivos, puntos_ponderados, peso_total = [], 0.0, 0.0

    def registrar(score, peso, motivo_si_no_verde=None):
        nonlocal puntos_ponderados, peso_total
        puntos_ponderados += score * peso
        peso_total += peso
        if score < 2 and motivo_si_no_verde: motivos.append(motivo_si_no_verde)

    duraciones_7d = []
    for c in checkins_7d:
        try:
            if str(c.get("duration_min", "")).strip() != "": duraciones_7d.append(float(c["duration_min"]))
        except (ValueError, TypeError): pass
    if duraciones_7d:
        if any(d < 30 for d in duraciones_7d): registrar(0, 2, "Alguna sesión de esta semana duró menos de 30 min")
        elif any(d <= 45 for d in duraciones_7d): registrar(1, 2, "Alguna sesión de esta semana duró entre 30 y 45 min")
        else: registrar(2, 2)

    evas_7d = [float(c.get("eva", 0)) for c in checkins_7d if c.get("eva", "")]
    if evas_7d:
        peor_eva = max(evas_7d)
        if peor_eva >= 7: registrar(0, 2, f"Dolor alto en alguna sesión de esta semana (EVA {int(peor_eva)}/10)")
        elif peor_eva >= 4: registrar(1, 2, f"Dolor moderado en alguna sesión de esta semana (EVA {int(peor_eva)}/10)")
        else: registrar(2, 2)

    try:
        delta_eva = float(checkins_ordenados[0].get("eva", 0)) - float(checkins_ordenados[1].get("eva", 0))
        if delta_eva >= 2: registrar(0, 1, f"Subida de dolor entre las dos últimas sesiones (+{int(delta_eva)} puntos de EVA)")
        else: registrar(2, 1)
    except (ValueError, TypeError, IndexError): pass

    try:
        borgs_ultimas_3 = [float(c.get("borg", 0)) for c in checkins_ordenados[:3]]
    ...
    if n_checkins_7d < 2: registrar(0, 1, f"Menos de 2 sesiones esta semana ({n_checkins_7d})")
    elif n_checkins_7d == 2: registrar(1, 1, "Solo 2 sesiones esta semana")
    else: registrar(2, 1)

    if peso_total == 0: return resultado("AMARILLO", ["No hay datos suficientes de esta semana para calcular la adherencia"])
    media = puntos_ponderados / peso_total
    estado = "VERDE" if media >= 1.5 else "AMARILLO" if media >= 0.75 else "ROJO"
...
if st.session_state.admin_mode:
    ...
    def modal_editar_sesion(pl):
        ...
        for cat in CATEGORIAS_DB:
            ...
            if filtro_cat_ed != "Todas":
                if get_main_category(cat) != filtro_cat_ed: mostrar_cat = False
            ...
            ej_cat = sorted([x for x in active_exercises if x.get("category") == cat], key=lambda x: x["name"].lower())
            for e in ej_cat:
                nk = f"{cat}  |  {e['name']}"
        ...
        opciones_ind = {item["text"]: item["id"] for item in active_instructions}
        default_ind = [item["text"] for item in active_instructions if item["id"] in pl.get("generalInstructionIds", [])]
        ...
        if st.session_state[f"edit_ses_{pl_id}_ejs"]:
            ...
            for idx, e_id in enumerate(st.session_state[f"edit_ses_{pl_id}_ejs"]):
                ...
                with c_t: st.markdown(f"<div style='margin-top:8px; font-size:14px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;'>{idx+1}. {ej_name}</div>", unsafe_allow_html=True)
                with c_s: s = st.text_input("S", value=prev_inst.get("series", ""), key=f"es_{pl_id}_{e_id}", label_visibility="collapsed", placeholder="Series")
                with c_r: r = st.text_input("R", value=prev_inst.get("reps", ""), key=f"er_{pl_id}_{e_id}", label_visibility="collapsed", placeholder="Reps")
                with c_n:
    ...
    def modal_editar_programa(pr):
        pr_id = pr["id"]
        pac_idx = [p["id"] for p in active_patients].index(pr["patientId"]) if pr["patientId"] in [p["id"] for p in active_patients] else 0
        
        c1, c2 = st.columns([1, 2])
        af_paciente = c1.selectbox("1. Paciente:", options=[p["id"] for p in active_patients], format_func=get_patient_name, index=pac_idx)
        ...
        opciones_ind_af = {item["text"]: item["id"] for item in active_instructions}
        default_ind_af = [item["text"] for item in active_instructions if item["id"] in pr.get("generalInstructionIds", [])]
        ...
        if f"edit_af_{pr_id}_dias" not in st.session_state: st.session_state[f"edit_af_{pr_id}_dias"] = max(1, len(pr["daysData"]))

        cb1, cb2 = st.columns(2)
        if cb1.button("➕ Añadir Día al Plan"):
            st.session_state[f"edit_af_{pr_id}_dias"] += 1; st.rerun()
        ...
        for d_idx in range(st.session_state[f"edit_af_{pr_id}_dias"]):
            ...
            if cc1.button(f"➕ Añadir Bloque al Día {d_idx+1}", key=f"eaddB_{pr_id}_{d_idx}"):
                st.session_state[f"edit_af_{pr_id}_b_{d_idx}"] += 1; st.rerun()
            ...
            for b_idx in range(st.session_state[f"edit_af_{pr_id}_b_{d_idx}"]):
                with st.container(border=True):
                    ...
                    b_subcat = "Todos"
                    if b_cat == "EEII": b_subcat = c_sub.selectbox("Filtro EEII:", ["Todos", "EEII (General)", "EEII (3FE)", "EEII (H-H)"], key=f"ebsub_{pr_id}_{d_idx}_{b_idx}")
                    elif b_cat == "EESS": b_subcat = c_sub.selectbox("Filtro EESS:", ["Todos", "EESS (General)", "EESS (empuje)", "EESS (traccion)"], key=f"ebsub_{pr_id}_{d_idx}_{b_idx}")
                    ...
                    if f"edit_af_{pr_id}_ejs_{d_idx}_{b_idx}" not in st.session_state:
                        default_ids = [ex["exerciseId"] for ex in old_block.get("exercises", [])] if old_block else []
                    ...
                    ej_options_block_sorted = {k: ej_options_block[k] for k in sorted(ej_options_block.keys(), key=lambda x: x.lower())}
                    ...
                    for idx_e, eid in enumerate(st.session_state[f"edit_af_{pr_id}_ejs_{d_idx}_{b_idx}"]):
                        ...
                        def_prio, def_s, def_r, def_n = False, "", "", ej_obj.get("defaultNote", "") if ej_obj else ""
                        
                        if old_block:
                            for ex_old in old_block.get("exercises", []):
                                if ex_old.get("exerciseId") == eid:
                                    def_prio, def_s, def_r, def_n = ex_old.get("isPriority", False), ex_old.get("series", ""), ex_old.get("reps", ""), ex_old.get("notes", "")
                                    break
                                    
                        prio_key = f"eprio_{pr_id}_{d_idx}_{b_idx}_{eid}"
                        if prio_key not in st.session_state: st.session_state[prio_key] = def_prio
                            
                        ce1, ce2, ce3, ce4, ce5, ce6, ce7 = st.columns([3, 1, 1, 2, 1.5, 0.6, 0.6])
                        with ce1: st.markdown(f"<div style='margin-top:6px; font-weight:bold; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;'>{idx_e+1}. {ename}</div>", unsafe_allow_html=True)
                        with ce2: s = st.text_input("S", value=def_s, key=f"es_af_{pr_id}_{d_idx}_{b_idx}_{eid}", placeholder="Ser", label_visibility="collapsed")
                        with ce3: r = st.text_input("R", value=def_r, key=f"er_af_{pr_id}_{d_idx}_{b_idx}_{eid}", placeholder="Rep", label_visibility="collapsed")
                        with ce4: n = st.text_input("N", value=def_n, key=f"en_af_{pr_id}_{d_idx}_{b_idx}_{eid}", placeholder="Nota", label_visibility="collapsed")
                        with ce5: es_prio = st.checkbox("⭐ Prio", key=prio_key)
                        with ce6:
                    ...
                    bloques_dia.append({"blockTitle": f"{b_cat} {b_regla}".strip(), "blockCategory": b_cat, "blockRule": b_regla, "exercises": ejs_bloque_info})
            dias_construidos.append({"dayTitle": d_titulo, "blocks": bloques_dia})
    ...
    menu_seleccion = st.sidebar.radio("Panel de Control:", ["📁 Archivo", "🩺 Sesiones", "🏋️ Programas de AF"])
    ...
    # Backup JSON siempre incluirá todos los registros, incluso los soft-deleted, para mayor seguridad histórica.
    backup_data = {"pacientes": patients, "ejercicios": exercises, "indicaciones_generales": general_instructions, "sesiones": plans, "programas_af": programs_af, "checkins": checkins}
    ...
    if st.session_state.gsheets_read_error:
        st.error("⚠️ Atención: Ha habido un fallo de conexión con Google Sheets. Las funciones de guardado están bloqueadas temporalmente para proteger tus datos. Recarga la página en unos segundos.")
    ...
    if menu_seleccion == "📁 Archivo":
        ...
        with tab_pac:
                    ...
                    if st.form_submit_button("Guardar Paciente Nuevo", type="primary"):
                        if new_p_name:
                            ...
                            if new_p_review_input.strip():
                                try: review_str = datetime.datetime.strptime(new_p_review_input.strip(), "%d/%m/%Y").strftime("%Y-%m-%d")
                                except ValueError: fecha_valida = False
                            if not fecha_valida:
                                st.error("⚠️ La fecha de revisión no es válida. Usa el formato DD/MM/AAAA (ej: 15/03/2026).")
                            else:
                                new_p = {"id": str(uuid.uuid4()), "name": new_p_name, "phone": new_p_phone, "lesion": new_p_lesion, "review_date": review_str, "revisions": [], "is_deleted": False}
                                patients.append(new_p)
                                active_patients.append(new_p)
                                upsert_patient(new_p)
                                st.success("¡Paciente añadido y sincronizado!")
                                st.rerun()

            total_pacs = len(active_patients)
            ...
            for p in pacs_filtrados:
                ...
                with st.expander(titulo_exp):
                    ...
                    sesiones_del_paciente = [pl for pl in active_plans if str(pl["patientId"]) == str(p["id"])]
                    progs_del_paciente = [pr for pr in active_programs_af if str(pr["patientId"]) == str(p["id"])]
                    
                    if sesiones_del_paciente:
                        st.markdown("**Sesiones Clínicas:**")
                        for pi in reversed(sesiones_del_paciente): st.write(f"- {pi['title']} (PIN: {pi['pin']})")
                    if progs_del_paciente:
                        st.markdown("**Programas de AF:**")
                        for pr in reversed(progs_del_paciente): st.write(f"- {pr['title']} (PIN: {pr['pin']})")
                    ...
                    try: curr_rev_display = datetime.datetime.strptime(p.get("review_date", ""), "%Y-%m-%d").strftime("%d/%m/%Y")
                    except: curr_rev_display = ""
                    ...
                    if st.button("💾 Actualizar Datos", key=f"upd_{p['id']}", type="primary"):
                        ...
                        if edit_review_input.strip():
                            try: nueva_review_str = datetime.datetime.strptime(edit_review_input.strip(), "%d/%m/%Y").strftime("%Y-%m-%d")
                            except ValueError: fecha_valida = False
                        if not fecha_valida: st.error("⚠️ La fecha de revisión no es válida. Usa el formato DD/MM/AAAA (ej: 15/03/2026).")
                        else:
                            p["name"] = edit_name; p["phone"] = edit_phone; p["lesion"] = edit_lesion; p["review_date"] = nueva_review_str
                            upsert_patient(p)
                            st.rerun()
                    ...
                    if not p["revisions"]: st.caption("Este paciente todavía no tiene ninguna revisión registrada.")

                    for r_idx, rev in enumerate(p["revisions"]):
                        try: fecha_rev_disp = datetime.datetime.strptime(rev["date"], "%Y-%m-%d").strftime("%d/%m/%Y") if rev.get("date") else ""
                        except: fecha_rev_disp = ""

                        col_exp, col_fecha_rev = st.columns([8, 1.3])
                        with col_fecha_rev: rev_fecha_input = st.text_input("Fecha", value=fecha_rev_disp, key=f"rev_fecha_{p['id']}_{r_idx}", placeholder="DD/MM/AAAA", label_visibility="collapsed")
                        with col_exp:
                            with st.expander(f"📋 {ordinal_revision(r_idx + 1)}"):
                                ...
                                if col_save_rev.button("💾 Guardar revisión", key=f"save_rev_{p['id']}_{r_idx}", type="primary", use_container_width=True):
                                    fecha_rev_valida, nueva_fecha_rev = True, ""
                                    if rev_fecha_input.strip():
                                        try: nueva_fecha_rev = datetime.datetime.strptime(rev_fecha_input.strip(), "%d/%m/%Y").strftime("%Y-%m-%d")
                                        except ValueError: fecha_rev_valida = False
                                    if not fecha_rev_valida: st.error("⚠️ La fecha no es válida. Usa el formato DD/MM/AAAA.")
                                    else:
                                        p["revisions"][r_idx] = {"date": nueva_fecha_rev, "anamnesis": rev_ana, "inspeccion": rev_ins, "movilidad": rev_mov, "fuerza": rev_fue}
                                        upsert_patient(p); st.success("Revisión guardada."); st.rerun()
                                if col_del_rev.button("🗑️ Eliminar revisión", key=f"del_rev_{p['id']}_{r_idx}", use_container_width=True):
                                    p["revisions"].pop(r_idx)
                                    upsert_patient(p); st.rerun()
                    ...
                    if st.session_state.confirm_delete_patient_id == p["id"]:
                        st.warning("Vas a borrar definitivamente este paciente. Esta acción ocultará todos sus datos en la app.")
                        ...
                        if confirm_col.button("Sí, borrar todos sus datos", key=f"confirm_del_patient_{p['id']}", type="primary", use_container_width=True):
                            # Soft Delete en Cascada (Actualizaciones atómicas)
                            patient_id = str(p["id"])
                            p["is_deleted"] = True
                            upsert_patient(p)
                            
                            for pl in active_plans:
                                if str(pl["patientId"]) == patient_id:
                                    pl["is_deleted"] = True; upsert_plan(pl)
                            for pr in active_programs_af:
                                if str(pr["patientId"]) == patient_id:
                                    pr["is_deleted"] = True; upsert_program_af(pr)
                        ...
                        if cancel_col.button("Cancelar", key=f"cancel_del_patient_{p['id']}", use_container_width=True):
                            st.session_state.confirm_delete_patient_id = None; st.rerun()
                    elif st.button("🗑️ Borrar Paciente", key=f"del_{p['id']}"):
                        st.session_state.confirm_delete_patient_id = p["id"]; st.rerun()

        with tab_ej:
            total_ej = len(active_exercises)
            ...
            with st.form("form_añadir_ejercicio", clear_on_submit=True):
                ...
                with cn1: new_n = st.text_input("new_n", placeholder="Nombre del ejercicio...", label_visibility="collapsed")
                with cn_alt: new_pn = st.text_input("new_pn", placeholder="Nombre para el paciente (opcional)...", label_visibility="collapsed")
                with cn2: new_u = st.text_input("new_u", placeholder="Enlace de YouTube...", label_visibility="collapsed")
                with cn3: new_c = st.selectbox("new_c", CATEGORIAS_DB, label_visibility="collapsed")
                with cn4: btn_add = st.form_submit_button("➕ Añadir", type="primary", use_container_width=True)
                
                if btn_add:
                    if new_n.strip():
                        new_ex = {"id": str(uuid.uuid4()), "name": new_n.strip(), "patientName": new_pn.strip(), "videoUrl": new_u.strip(), "category": new_c, "defaultNote": "", "is_deleted": False}
                        exercises.append(new_ex); active_exercises.append(new_ex)
                        upsert_exercise(new_ex)
                        st.success("¡Ejercicio añadido a la base de datos!")
                        st.rerun()
                    else: st.warning("⚠️ El nombre del ejercicio es obligatorio.")
            st.write("")
            
            with st.container():
                btn_save = st.button("💾 Guardar Todos los Cambios", type="primary", use_container_width=True, key="save_all_exercises")
                st.markdown("<hr style='margin: 10px 0 20px 0;'>", unsafe_allow_html=True)
                nuevos_datos, ids_borrar = {}, []
                c_h1, c_h_comment, c_h_alt, c_h2, c_h3, c_h4 = st.columns([2.7, 1.0, 2.0, 2.5, 1.3, 1.0])
                c_h1.caption("NOMBRE"); c_h_alt.caption("NOMBRE PARA EL PACIENTE"); c_h2.caption("ENLACE YOUTUBE"); c_h3.caption("CATEGORÍA"); c_h4.caption("ACCIÓN")
                
                for cat in CATEGORIAS_EJ:
                    ej_cat = sorted([e for e in active_exercises if get_main_category(e.get("category")) == cat], key=lambda x: x["name"].lower())
                    if ej_cat:
                        ...
                        for e in ej_cat:
                            ...
                            with c1: n = st.text_input("n", value=e["name"], key=f"n_{eid}", label_visibility="collapsed")
                            with c_alt: pn = st.text_input("pn", value=e.get("patientName", ""), key=f"pn_{eid}", label_visibility="collapsed", placeholder="(igual que el nombre)")
                            with c2: u = st.text_input("u", value=e["videoUrl"], key=f"u_{eid}", label_visibility="collapsed")
                            with c3:
                            ...
                            with c_comment:
                                ...
                                with st.popover(f"📝 {icono_estado}", help="Abrir comentario asociado a este ejercicio"):
                                    st.markdown("""<div style="position:relative; background:#fffef7; border:2px solid #13765d; border-radius:20px 20px 20px 7px; padding:14px 17px 15px 17px; margin:0 0 12px 0; color:#103d33; font-size:13px; line-height:1.5; box-shadow:0 5px 15px rgba(16,61,51,.12);">
                                    ...
                                    <div style="width:0; height:0; border-top:12px solid #13765d; border-right:12px solid transparent; margin:-12px 0 10px 18px;"></div>
                                    """, unsafe_allow_html=True)
                                    st.text_area("Comentario", value=e.get("defaultNote", ""), key=f"default_note_{eid}", height=130, placeholder="Ej: Mantén la rodilla alineada...", label_visibility="collapsed")
                            with c4: b = st.checkbox("🗑️ Borrar", key=f"del_{eid}")
                                
                            nuevos_datos[eid] = {"name": n, "patientName": pn, "videoUrl": u, "category": c, "defaultNote": st.session_state.get(f"default_note_{eid}", e.get("defaultNote", "")).strip()}
                ...
                if btn_save:
                    ...
                    if exercise_ids_to_delete:
                        for plan in active_plans:
                            if any(str(eid) in exercise_ids_to_delete for eid in plan.get("exerciseIds", [])): references.append(f"Sesión: {plan['title']} ({get_patient_name(plan['patientId'])})")
                        for program in active_programs_af:
                            if any(str(item.get("exerciseId", "")) in exercise_ids_to_delete for day in program.get("daysData", []) for block in day.get("blocks", []) for item in block.get("exercises", [])):
                    ...
                    if references:
                        ...
                        for e in exercises:
                            eid = e["id"]
                            if eid in exercise_ids_to_delete: e["is_deleted"] = True
                            elif eid in nuevos_datos: e.update(nuevos_datos[eid])
                        save_exercises_bulk(exercises)
                        st.success("¡Base de datos de ejercicios actualizada!")
                        st.rerun()

        with tab_ind:
            st.markdown(f"<h3 style='margin-top:10px;'>📌 Banco de Indicaciones Generales (Total: {len(active_instructions)})</h3>", unsafe_allow_html=True)
            with st.form("form_añadir_indicacion", clear_on_submit=True):
                texto_ind = st.text_area("Nueva indicación", placeholder="Ej: Realiza los ejercicios con una técnica controlada...", height=90, label_visibility="collapsed")
                if st.form_submit_button("➕ Añadir indicación", type="primary", use_container_width=True):
                    txt = texto_ind.strip()
                    if not txt: st.warning("⚠️ La indicación no puede estar vacía.")
                    elif any(x["text"].strip().lower() == txt.lower() for x in active_instructions): st.warning("⚠️ Esa indicación ya existe.")
                    else:
                        new_ind = {"id": str(uuid.uuid4()), "text": txt, "is_deleted": False}
                        general_instructions.append(new_ind); active_instructions.append(new_ind)
                        upsert_general_instruction(new_ind); st.rerun()
            st.write("")
            if not active_instructions: st.info("Todavía no has creado ninguna indicación general.")
            else:
                for idx_ind, item in enumerate(active_instructions, 1):
                    with st.container(border=True): st.text_area(f"Indicación {idx_ind}", value=item["text"], key=f"ind_text_{item['id']}", height=90)
                if st.button("💾 Guardar cambios del listado", type="primary", use_container_width=True):
                    updated, seen, valid = {}, set(), True
                    for item in active_instructions:
                        txt = st.session_state.get(f"ind_text_{item['id']}", item["text"]).strip()
                        if not txt or txt.lower() in seen: valid = False; break
                        seen.add(txt.lower()); updated[item["id"]] = txt
                    if valid:
                        for i in general_instructions:
                            if i["id"] in updated: i["text"] = updated[i["id"]]
                        save_general_instructions_bulk(general_instructions); st.success("¡Listado actualizado!"); st.rerun()
        ...
        with tab_ses_act:
            if st.session_state.get("editing_sesion_id"):
                target_ses = next((pl for pl in active_plans if str(pl["id"]) == str(st.session_state.editing_sesion_id)), None)
                if target_ses: modal_editar_sesion(target_ses)
                else: st.session_state.editing_sesion_id = None

            search_query = st.text_input("🔍 Buscar sesión por título o nombre del paciente:")
            sesiones_actuales = {pl["patientId"]: pl for pl in active_plans if pl.get("isActive", True)}
            ...
            if not planes_filtrados: st.info("No se encontraron sesiones.")
            else:
                    ...
                    with st.container(border=True):
                        ...
                        with c_del:
                            if st.session_state.confirm_delete_session_id == pl["id"]:
                                st.warning("Se borrará la sesión. El código de acceso dejará de funcionar.")
                                confirm_col, cancel_col = st.columns(2)
                                if confirm_col.button("Sí, borrar sesión", key=f"confirm_del_session_{pl['id']}", type="primary", use_container_width=True):
                                    pl["is_deleted"] = True
                                    upsert_plan(pl)
                                    st.session_state.confirm_delete_session_id = None; st.rerun()
                                ...
                                st.session_state.confirm_delete_session_id = pl["id"]; st.rerun()

        with tab_crear_ses:
            if not active_patients: st.warning("Añade pacientes en el apartado 'Archivo' primero.")
            else:
                paciente_sel = st.selectbox("1. Paciente:", options=[p["id"] for p in active_patients], format_func=get_patient_name)
                ...
                filtro_subcat = "Todos"
                if filtro_cat == "EEII": filtro_subcat = c_filt2.selectbox("Subcategoría EEII:", ["Todos", "EEII (General)", "EEII (3FE)", "EEII (H-H)"], key="crear_ses_fsub")
                elif filtro_cat == "EESS": filtro_subcat = c_filt2.selectbox("Subcategoría EESS:", ["Todos", "EESS (General)", "EESS (empuje)", "EESS (traccion)"], key="crear_ses_fsub")
                
                if 'orden_ejs' not in st.session_state: st.session_state.orden_ejs = []
                nombres_actuales = [f"{get_exercise(eid)['category']}  |  {get_exercise(eid)['name']}" for eid in st.session_state.orden_ejs if get_exercise(eid)]
                
                ej_options = {}
                for cat in CATEGORIAS_DB:
                    mostrar_cat = True
                    if filtro_cat != "Todas":
                        if get_main_category(cat) != filtro_cat: mostrar_cat = False
                    ...
                    ej_ordenados = sorted([x for x in active_exercises if x.get("category") == cat], key=lambda x: x["name"].lower())
                    for e in ej_ordenados:
                        name_key = f"{cat}  |  {e['name']}"
                        if mostrar_cat or (name_key in nombres_actuales): ej_options[name_key] = e['id']
                ...
                for e in ejs_seleccionados:
                    if e not in st.session_state.orden_ejs: st.session_state.orden_ejs.append(e)
                
                opciones_ind_crear = {item["text"]: item["id"] for item in active_instructions}
                ...
                if st.session_state.orden_ejs:
                    ...
                    c_th.caption("EJERCICIO"); c_sh.caption("SERIES"); c_rh.caption("REPS"); c_nh.caption("NOTAS EXTRA")
                    ...
                    for idx, e_id in enumerate(st.session_state.orden_ejs):
                        ...
                        if ej_obj and not tiene_video_valido(ej_obj.get("videoUrl", "")):
                            sin_video = True; has_missing_videos = True; ej_name += " ⚠️ (SIN VÍDEO)"

                        col_t, col_s, col_r, col_n, col_up, col_dn = st.columns([4, 1.5, 1.5, 2.5, 0.6, 0.6])
                        with col_t: st.markdown(f"<div style='margin-top:8px; font-weight:bold; font-size:14px; color:{'#aa3838' if sin_video else 'inherit'}; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;'>{idx + 1}. {ej_name}</div>", unsafe_allow_html=True)
                        with col_s: s = st.text_input("S", key=f"ser_{e_id}", placeholder="Series", label_visibility="collapsed")
                        with col_r: r = st.text_input("R", key=f"rep_{e_id}", placeholder="Reps", label_visibility="collapsed")
                        with col_n:
                            note_key = f"not_{e_id}"
                            if note_key not in st.session_state: st.session_state[note_key] = (ej_obj.get("defaultNote", "") if ej_obj else "")
                        ...
                        with col_up:
                            if st.button("⬆️", key=f"up_{e_id}") and idx > 0:
                                ...
                                st.rerun()
                        with col_dn:
                            if st.button("⬇️", key=f"dn_{e_id}") and idx < len(st.session_state.orden_ejs) - 1:
                ...
                st.write("")
                if has_missing_videos: st.warning("⚠️ Atención: Hay ejercicios en la lista sin vídeo asignado.")

                if st.button("💾 Generar Sesión", type="primary"):
                    if not titulo_sesion.strip(): st.warning("⚠️ Introduce un título para la sesión.")
                    elif not st.session_state.orden_ejs: st.warning("⚠️ Debes seleccionar al menos un ejercicio.")
                    else:
                        nuevo_pin = generate_unique_access_code(active_plans, active_programs_af)
                        # Atomic Upsert para desactivar las antiguas y crear la nueva
                        for pl in active_plans:
                            if str(pl["patientId"]) == str(paciente_sel) and pl.get("isActive"):
                                pl["isActive"] = False
                                upsert_plan(pl)
                        ...
                        nuevo_plan = {
                            ...
                            "pin": nuevo_pin, "startDate": datetime.date.today().strftime("%d/%m/%Y"),
                            "isActive": True, "is_deleted": False
                        }
                        plans.append(nuevo_plan); active_plans.append(nuevo_plan)
                        upsert_plan(nuevo_plan)
                        st.session_state.orden_ejs = []
                        st.success("¡Sesión guardada!")
                        
                        mensaje_whatsapp = f"¡Hola {get_patient_name(paciente_sel)}! 👋\n\nAquí tienes tu nueva sesión de fisioterapia: *{titulo_sesion}*.\n\n📱 Pulsa en el enlace para entrar directamente:\n{APP_URL}/?pin={nuevo_pin}\n\n¡A por ello!"
        ...
        with tab_checkins:
            ...
            if not checkins: st.info("Aún no hay reportes registrados por pacientes para sus sesiones.")
            else:
        ...
        with tab_seguimiento:
            ...
            if not active_patients: st.warning("Añade pacientes en el apartado 'Archivo' primero.")
                ...
                paciente_sel_seg = st.selectbox("🔍 Buscar Paciente:", options=[p["id"] for p in active_patients], format_func=get_patient_name, key="search_pac_seg")
                paciente_obj_seg = next((pp for pp in active_patients if str(pp["id"]) == str(paciente_sel_seg)), None)
                tiene_sesiones_clinicas_seg = any(str(pl["patientId"]) == str(paciente_sel_seg) for pl in active_plans)

                if paciente_obj_seg and tiene_sesiones_clinicas_seg:
                    res_semaforo = calcular_semaforo_paciente(paciente_obj_seg, active_plans, checkins)
                ...
                planes_paciente = [p["id"] for p in active_plans if str(p["patientId"]) == str(paciente_sel_seg)]
                checkins_paciente = [c for c in checkins if str(c["planId"]) in planes_paciente]
                
                if not checkins_paciente: st.info("Este paciente aún no ha registrado ningún reporte en sus sesiones.")
                else:
                    ...
                    df_melted = df_checkins[['Fecha_Corta', 'eva', 'borg']].copy().rename(columns={'eva': 'Dolor (EVA)', 'borg': 'Fatiga (Borg)'}).melt('Fecha_Corta', var_name='Métrica', value_name='Puntuación')
                    grafica = alt.Chart(df_melted).mark_line(point=True).encode(
                        ...
                        color=alt.Color('Métrica:N', scale=alt.Scale(domain=['Dolor (EVA)', 'Fatiga (Borg)'], range=["#aa3838", "#13765d"]))
                    ).properties(height=350)
                    
                    st.altair_chart(grafica, use_container_width=True)
                    st.markdown("#### 💬 Historial de Comentarios")
                    df_comments = df_checkins[['Fecha_Larga', 'eva', 'borg', 'comment']].copy().rename(columns={'Fecha_Larga': 'Fecha', 'eva': 'EVA', 'borg': 'Borg', 'comment': 'Comentario'})
        ...
        with tab_gest_af:
            if st.session_state.get("editing_program_id"):
                target_pr = next((pr for pr in active_programs_af if str(pr["id"]) == str(st.session_state.editing_program_id)), None)
                if target_pr: modal_editar_programa(target_pr)
                else: st.session_state.editing_program_id = None

            search_query_af = st.text_input("🔍 Buscar programa por título o nombre del paciente:")
            progs_filtrados = list(reversed(active_programs_af))
            ...
            if not progs_filtrados: st.info("No hay programas de AF que coincidan con la búsqueda.")
            else:
                    ...
                    with st.container(border=True):
                        ...
                        with c_del:
                            if st.session_state.confirm_delete_program_id == pr["id"]:
                                ...
                                st.session_state.confirm_delete_program_id = pr["id"]; st.rerun()

        with tab_crear_af:
            if not active_patients: st.warning("Añade pacientes en el apartado 'Archivo' primero.")
            else:
                if "prio_dict" not in st.session_state: st.session_state.prio_dict = {}
                def _save_prio(k): st.session_state.prio_dict[k] = st.session_state[k]

                st.markdown("### 📝 Diseñador de Programa Semanal de AF")
                col_p, col_t = st.columns([1, 2])
                af_paciente = col_p.selectbox("1. Paciente:", options=[p["id"] for p in active_patients], format_func=get_patient_name, key="af_pac")
                ...
                opciones_ind_af_crear = {item["text"]: item["id"] for item in active_instructions}
                ...
                if col_btn_d1.button("➕ Añadir Día al Plan"): st.session_state.num_dias += 1; st.rerun()
                if col_btn_d2.button("➖ Quitar Último Día") and st.session_state.num_dias > 1: st.session_state.num_dias -= 1; st.rerun()

                dias_construidos, falta_algun_video_af = [], False
                if "af_block_clipboard" not in st.session_state: st.session_state.af_block_clipboard = None
                ...
                for d_idx in range(st.session_state.num_dias):
                    ...
                    if key_num_bloques not in st.session_state: st.session_state[key_num_bloques] = 1

                    cb1, cb2, cb3 = st.columns(3)
                    if cb1.button(f"➕ Añadir Bloque al Día {d_idx + 1}", key=f"addb_{d_idx}"): st.session_state[key_num_bloques] += 1; st.rerun()
                    if cb2.button(f"➖ Quitar Bloque al Día {d_idx + 1}", key=f"delb_{d_idx}") and st.session_state[key_num_bloques] > 1: st.session_state[key_num_bloques] -= 1; st.rerun()

                    pegar_bloque = cb3.button("📥 Pegar bloque copiado", key=f"pasteb_{d_idx}", disabled=st.session_state.af_block_clipboard is None, help="Añade al final de este día una copia exacta del último bloque que hayas copiado.")
                    if pegar_bloque and st.session_state.af_block_clipboard is not None:
                        ...
                        st.session_state.af_pending_paste = {"d_idx": d_idx, "b_idx": nuevo_b_idx, "block": deepcopy(st.session_state.af_block_clipboard)}
                        st.rerun()

                    bloques_dia = []
                    for b_idx in range(st.session_state[key_num_bloques]):
                        pending_paste = st.session_state.get("af_pending_paste")
                        if pending_paste and pending_paste.get("d_idx") == d_idx and pending_paste.get("b_idx") == b_idx:
                            pasted_block = pending_paste["block"]
                            pasted_category = pasted_block.get("blockCategory", CATEGORIAS_EJ[0])
                            if pasted_category not in CATEGORIAS_EJ: pasted_category = CATEGORIAS_EJ[0]

                            st.session_state[f"bcat_{d_idx}_{b_idx}"] = pasted_category
                            st.session_state[f"breg_{d_idx}_{b_idx}"] = pasted_block.get("blockRule", "")
                            pasted_ids = [ex.get("exerciseId") for ex in pasted_block.get("exercises", []) if ex.get("exerciseId") and get_exercise(ex.get("exerciseId"))]
                            st.session_state[f"orden_af_{d_idx}_{b_idx}"] = pasted_ids.copy()
                            st.session_state[f"bejs_{d_idx}_{b_idx}"] = [get_exercise(eid)["name"] for eid in pasted_ids if get_exercise(eid) and get_main_category(get_exercise(eid).get("category")) == pasted_category]

                            for ex in pasted_block.get("exercises", []):
                                eid = ex.get("exerciseId")
                                if not eid or not get_exercise(eid): continue
                        ...
                        with st.container(border=True):
                            ...
                            b_subcat = "Todos"
                            if b_cat == "EEII": b_subcat = col_bc2.selectbox("Filtro EEII:", ["Todos", "EEII (General)", "EEII (3FE)", "EEII (H-H)"], key=f"bsub_{d_idx}_{b_idx}")
                            ...
                            if key_orden not in st.session_state: st.session_state[key_orden] = []
                            ...
                            for e in active_exercises:
                                ...
                                if get_main_category(cat_e) == b_cat:
                                    ...
                                    if mostrar: ej_options_block[e["name"]] = e["id"]
                            ...
                            for e in new_ids:
                                if e not in st.session_state[key_orden]: st.session_state[key_orden].append(e)
                            ...
                            for idx_e, eid in enumerate(st.session_state[key_orden]):
                                ...
                                sin_video = False
                                if ej_obj and not tiene_video_valido(ej_obj.get("videoUrl", "")):
                                    sin_video = True; falta_algun_video_af = True; ename += " ⚠️ (SIN VÍDEO)"

                                prio_k = f"prio_{d_idx}_{b_idx}_{eid}"
                                note_k = f"naf_{d_idx}_{b_idx}_{eid}"
                                if note_k not in st.session_state and ej_obj: st.session_state[note_k] = ej_obj.get("defaultNote", "")

                                ce1, ce2, ce3, ce4, ce5, ce6, ce7 = st.columns([3, 1, 1, 2, 1.5, 0.6, 0.6])
                                with ce1: st.markdown(f"<div style='margin-top:6px; font-weight:bold; color:{'#aa3838' if sin_video else 'inherit'}; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;'>{idx_e+1}. {ename}</div>", unsafe_allow_html=True)
                                with ce2: s = st.text_input("S", key=f"saf_{d_idx}_{b_idx}_{eid}", placeholder="Series", label_visibility="collapsed")
                                with ce3: r = st.text_input("R", key=f"raf_{d_idx}_{b_idx}_{eid}", placeholder="Reps", label_visibility="collapsed")
                                with ce4: n = st.text_input("N", key=note_k, placeholder="Notas", label_visibility="collapsed")
                                with ce5: es_prio = st.checkbox("⭐ Prio", key=prio_k, on_change=_save_prio, args=(prio_k,))
                                with ce6:
                            ...
                            if st.button("📋 Copiar este bloque", key=f"copyb_{d_idx}_{b_idx}"):
                                st.session_state.af_block_clipboard = {"blockTitle": f"{b_cat} {b_regla}".strip(), "blockCategory": b_cat, "blockRule": b_regla, "exercises": deepcopy(ejs_bloque_info)}
                ...
                st.write("")
                if falta_algun_video_af: st.warning("⚠️ Atención: Hay ejercicios en la lista marcados en rojo que no tienen vídeo asignado en la base de datos.")

                if st.button("💾 Guardar Programa de AF", type="primary"):
                    if not af_titulo.strip(): st.warning("⚠️ Debes asignar un título al programa.")
                    else:
                        nuevo_pin_af = generate_unique_access_code(active_plans, active_programs_af)
                        nuevo_pr = {
                            "id": str(uuid.uuid4()), "patientId": af_paciente, "title": af_titulo,
                            "frequency": af_frecuencia, "duration": af_duracion,
                            "generalInstructionIds": selected_general_instruction_ids_af_crear,
                            "generalNote": af_nota_gen, "pin": nuevo_pin_af,
                            "startDate": datetime.date.today().strftime("%d/%m/%Y"),
                            "daysData": dias_construidos, "is_deleted": False
                        }
                        programs_af.append(nuevo_pr); active_programs_af.append(nuevo_pr)
                        upsert_program_af(nuevo_pr)
                        st.success("¡Programa de AF guardado exitosamente!")
                        mensaje_wa = f"¡Hola {get_patient_name(af_paciente)}! 👋\n\nAquí tienes tu nuevo programa de actividad física: *{af_titulo}*.\n\n📱 Pulsa en el enlace para acceder directamente:\n{APP_URL}/?pin={nuevo_pin_af}\n\n¡A darle caña!"
    ...
    if isinstance(pin_param, list): pin_param = pin_param[0] if pin_param else ""

    if pin_param and not st.session_state.logged_pin: st.session_state.logged_pin = pin_param
    ...
    if not st.session_state.logged_pin:
                ...
                if btn_login:
                    ...
                    if login_is_temporarily_locked():
                        ...
                        if admin_pin and access_code_matches(raw_pin, admin_pin):
                            ...
                            matched_plan = next((p for p in active_plans if access_code_matches(raw_pin, p.get("pin", ""))), None)
                            matched_prog = None
                            if not matched_plan: matched_prog = next((pr for pr in active_programs_af if access_code_matches(raw_pin, pr.get("pin", ""))), None)
        ...
        plan_paciente = next((p for p in active_plans if access_code_matches(current_pin, p.get("pin", ""))), None)
        prog_paciente = next((pr for pr in active_programs_af if access_code_matches(current_pin, pr.get("pin", ""))), None)
        ...
        if not plan_paciente and not prog_paciente:
            ...
            if st.button("🔑 Introducir otro código"):
                st.session_state.logged_pin = None; st.rerun()
        elif plan_paciente:
            st.markdown(f"<h2 style='color:#13765d;'>{plan_paciente['title']}</h2>", unsafe_allow_html=True)
            st.markdown(f"**Paciente:** {get_patient_name(plan_paciente['patientId'])}")
            ...
            for idx, e_id in enumerate(plan_paciente["exerciseIds"], 1):
                ...
                series_val, reps_val, notes_val = inst.get("series", ""), inst.get("reps", ""), inst.get("notes", "")
                ...
                with st.container(border=True):
                    ...
                    with col_chk:
                        ...
                        if chk != is_checked:
                            if chk and e_id not in checked_list: checked_list.append(e_id)
                            elif not chk and e_id in checked_list: checked_list.remove(e_id)
                            guardar_progreso_sesion(p_id, progreso.get("opened_at", datetime.datetime.now().strftime("%Y-%m-%d %H:%M")), checked_list)
                            st.rerun()

                    with col_info:
                        st.markdown(f"#### {idx}. {nombre_para_paciente(ej_obj)}")
                        detalles = []
                        if series_val: detalles.append(f"**Series:** {series_val}")
                        if reps_val: detalles.append(f"**Reps:** {reps_val}")
                        if detalles: st.markdown(" | ".join(detalles))
                        if notes_val: st.markdown(f"💡 *{notes_val}*")

                    with col_vid:
                        vurl = ej_obj.get("videoUrl", "")
                        if tiene_video_valido(vurl): st.markdown(f"<div class='st-key-v_ses_{e_id}'><a href='{vurl}' target='_blank' style='display:inline-block; background:#13765d; color:white; padding:8px 14px; border-radius:8px; text-decoration:none; font-weight:bold; font-size:13px;'>▶️ Ver Vídeo</a></div>", unsafe_allow_html=True)
            ...
            with st.form(f"checkin_form_{p_id}", clear_on_submit=True):
                ...
                if st.form_submit_button("🚀 Enviar Reporte", type="primary", use_container_width=True):
                    save_checkin_item(p_id, datetime.datetime.now().strftime("%Y-%m-%d %H:%M"), eva_val, borg_val, comm_val, dur_val)
            ...
            if prog_paciente.get("generalNote"): st.info(prog_paciente["generalNote"])

            for d_idx, day in enumerate(prog_paciente.get("daysData", []), 1):
                ...
                for b in day.get("blocks", []):
                    with st.container(border=True):
                        ...
                        for ex_item in b.get("exercises", []):
                            ej_obj = get_exercise(ex_item.get("exerciseId"))
                            if not ej_obj: continue
                            
                            prio_str = "⭐ " if ex_item.get("isPriority") else ""
                            col_e1, col_e2 = st.columns([7, 3])
                            with col_e1:
                                st.markdown(f"**{prio_str}{nombre_para_paciente(ej_obj)}**")
                                det = []
                            ...
                            with col_e2:
                                vurl = ej_obj.get("videoUrl", "")
                                if tiene_video_valido(vurl): st.markdown(f"<a href='{vurl}' target='_blank' style='display:inline-block; background:#13765d; color:white; padding:6px 12px; border-radius:6px; text-decoration:none; font-weight:bold; font-size:12px;'>▶️ Ver Vídeo</a>", unsafe_allow_html=True)

        st.markdown("<br><br>", unsafe_allow_html=True)
        if st.button("🔒 Salir / Cambiar de código"):
            st.session_state.logged_pin = None; st.session_state.admin_mode = False; st.rerun()
